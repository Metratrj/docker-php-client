...
```php
buildPrune($keep_storage, $reserved_space, $max_used_space, $min_free_space, $all, $filters): \OpenAPI\Client\Model\BuildPruneResponse
```
...
$keep_storage = 56; // int | Amount of disk space in bytes to keep for cache  > **Deprecated**: This parameter is deprecated and has been renamed to \"reserved-space\". > It is kept for backward compatibility and will be removed in API v1.49.
$reserved_space = 56; // int | Amount of disk space in bytes to keep for cache
$max_used_space = 56; // int | Maximum amount of disk space allowed to keep for cache
$min_free_space = 56; // int | Target amount of free disk space after pruning
...
try {
    $result = $apiInstance->buildPrune($keep_storage, $reserved_space, $max_used_space, $min_free_space, $all, $filters);
    print_r($result);
} catch (Exception $e) {
...
| ------------- | ------------- | ------------- | ------------- |
| **keep_storage** | **int**| Amount of disk space in bytes to keep for cache  &gt; **Deprecated**: This parameter is deprecated and has been renamed to \&quot;reserved-space\&quot;. &gt; It is kept for backward compatibility and will be removed in API v1.49. | [optional] |
| **reserved_space** | **int**| Amount of disk space in bytes to keep for cache | [optional] |
| **max_used_space** | **int**| Maximum amount of disk space allowed to keep for cache | [optional] |
| **min_free_space** | **int**| Target amount of free disk space after pruning | [optional] |
...
```php
imageBuild($dockerfile, $t, $extrahosts, $remote, $q, $nocache, $cachefrom, $pull, $rm, $forcerm, $memory, $memswap, $cpushares, $cpusetcpus, $cpuperiod, $cpuquota, $buildargs, $shmsize, $squash, $labels, $networkmode, $content_type, $x_registry_config, $platform, $target, $outputs, $version, $input_stream)
```
...
$version = '1'; // string | Version of the builder backend to use.  - `1` is the first generation classic (deprecated) builder in the Docker daemon (default) - `2` is [BuildKit](https://github.com/moby/buildkit)
...
try {
    $apiInstance->imageBuild($dockerfile, $t, $extrahosts, $remote, $q, $nocache, $cachefrom, $pull, $rm, $forcerm, $memory, $memswap, $cpushares, $cpusetcpus, $cpuperiod, $cpuquota, $buildargs, $shmsize, $squash, $labels, $networkmode, $content_type, $x_registry_config, $platform, $target, $outputs, $version, $input_stream);
} catch (Exception $e) {
...
| **version** | **string**| Version of the builder backend to use.  - &#x60;1&#x60; is the first generation classic (deprecated) builder in the Docker daemon (default) - &#x60;2&#x60; is [BuildKit](https://github.com/moby/buildkit) | [optional] [default to &#39;1&#39;] |
...
| Name | Type | Description  | Notes |
| ------------- | ------------- | ------------- | ------------- |
| **from_image** | **string**| Name of the image to pull. If the name includes a tag or digest, specific behavior applies:  - If only &#x60;fromImage&#x60; includes a tag, that tag is used. - If both &#x60;fromImage&#x60; and &#x60;tag&#x60; are provided, &#x60;tag&#x60; takes precedence. - If &#x60;fromImage&#x60; includes a digest, the image is pulled by digest, and &#x60;tag&#x60; is ignored. - If neither a tag nor digest is specified, all tags are pulled. | [optional] |
...
```php
imageDelete($name, $force, $noprune, $platforms): \OpenAPI\Client\Model\ImageDeleteResponseItem[]
```
...
$platforms = array('platforms_example'); // string[] | Select platform-specific content to delete. Multiple values are accepted. Each platform is a OCI platform encoded as a JSON string.

try {
    $result = $apiInstance->imageDelete($name, $force, $noprune, $platforms);
    print_r($result);
} catch (Exception $e) {
...
| **platforms** | [**string[]**](../Model/string.md)| Select platform-specific content to delete. Multiple values are accepted. Each platform is a OCI platform encoded as a JSON string. | [optional] |
...
```php
imageGet($name, $platform): \SplFileObject
```
...
$name = 'name_example'; // string | Image name or ID
$platform = 'platform_example'; // string | JSON encoded OCI platform describing a platform which will be used to select a platform-specific image to be saved if the image is multi-platform. If not provided, the full multi-platform image will be saved.  Example: `{\"os\": \"linux\", \"architecture\": \"arm\", \"variant\": \"v5\"}`

try {
    $result = $apiInstance->imageGet($name, $platform);
    print_r($result);
} catch (Exception $e) {
...
| ------------- | ------------- | ------------- | ------------- |
| **name** | **string**| Image name or ID | |
| **platform** | **string**| JSON encoded OCI platform describing a platform which will be used to select a platform-specific image to be saved if the image is multi-platform. If not provided, the full multi-platform image will be saved.  Example: &#x60;{\&quot;os\&quot;: \&quot;linux\&quot;, \&quot;architecture\&quot;: \&quot;arm\&quot;, \&quot;variant\&quot;: \&quot;v5\&quot;}&#x60; | [optional] |
...
```php
imageGetAll($names, $platform): \SplFileObject
```
...
try {
    $result = $apiInstance->imageGetAll($names, $platform);
    print_r($result);
} catch (Exception $e) {
...
```php
imageHistory($name, $platform): \OpenAPI\Client\Model\HistoryResponseItem[]
```
...
```php
imageInspect($name, $manifests): \OpenAPI\Client\Model\ImageInspect
```
...
$name = 'name_example'; // string | Image name or id
$manifests = false; // bool | Include Manifests in the image summary.

try {
    $result = $apiInstance->imageInspect($name, $manifests);
    print_r($result);
} catch (Exception $e) {
...
| ------------- | ------------- | ------------- | ------------- |
| **name** | **string**| Image name or id | |
| **manifests** | **bool**| Include Manifests in the image summary. | [optional] [default to false] |
...
```php
imageList($all, $filters, $shared_size, $digests, $manifests): \OpenAPI\Client\Model\ImageSummary[]
```
...
try {
    $result = $apiInstance->imageList($all, $filters, $shared_size, $digests, $manifests);
    print_r($result);
} catch (Exception $e) {
...
```php
imageLoad($quiet, $platform, $images_tarball)
```
...
$platform = 'platform_example'; // string | JSON encoded OCI platform describing a platform which will be used to select a platform-specific image to be load if the image is multi-platform. If not provided, the full multi-platform image will be loaded.  Example: `{\"os\": \"linux\", \"architecture\": \"arm\", \"variant\": \"v5\"}`
$images_tarball = '/path/to/file.txt'; // \SplFileObject | Tar archive containing images

try {
    $apiInstance->imageLoad($quiet, $platform, $images_tarball);
} catch (Exception $e) {
...
| **platform** | **string**| JSON encoded OCI platform describing a platform which will be used to select a platform-specific image to be load if the image is multi-platform. If not provided, the full multi-platform image will be loaded.  Example: &#x60;{\&quot;os\&quot;: \&quot;linux\&quot;, \&quot;architecture\&quot;: \&quot;arm\&quot;, \&quot;variant\&quot;: \&quot;v5\&quot;}&#x60; | [optional] |
...
```php
imagePush($name, $x_registry_auth, $tag, $platform)
```
...
$name = 'name_example'; // string | Name of the image to push. For example, `registry.example.com/myimage`. The image must be present in the local image store with the same name.  The name should be provided without tag; if a tag is provided, it is ignored. For example, `registry.example.com/myimage:latest` is considered equivalent to `registry.example.com/myimage`.  Use the `tag` parameter to specify the tag to push.
$x_registry_auth = 'x_registry_auth_example'; // string | A base64url-encoded auth configuration.  Refer to the [authentication section](#section/Authentication) for details.
$tag = 'tag_example'; // string | Tag of the image to push. For example, `latest`. If no tag is provided, all tags of the given image that are present in the local image store are pushed.
...
try {
    $apiInstance->imagePush($name, $x_registry_auth, $tag, $platform);
} catch (Exception $e) {
...
| **tag** | **string**| Tag of the image to push. For example, &#x60;latest&#x60;. If no tag is provided, all tags of the given image that are present in the local image store are pushed. | [optional] |
...
$filters = 'filters_example'; // string | A JSON encoded value of the filters (a `map[string][]string`) to process on the images list. Available filters:  - `is-official=(true|false)` - `stars=<number>` Matches images that has at least 'number' stars.
...
| **filters** | **string**| A JSON encoded value of the filters (a &#x60;map[string][]string&#x60;) to process on the images list. Available filters:  - &#x60;is-official&#x3D;(true|false)&#x60; - &#x60;stars&#x3D;&lt;number&gt;&#x60; Matches images that has at least &#39;number&#39; stars. | [optional] |
